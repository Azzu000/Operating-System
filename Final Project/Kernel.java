      ...
      switch( irq ) {
         case INTERRUPT_SOFTWARE: // System calls
            switch( cmd ) { 
               ...
               //Note to self: FileTableEntry open( String filename, String mode )
                if((myTcb = scheduler.getMyTcb()) != null){
                    //store the open arguments given by args 
                    String[] openArgs = (String[]) args;
                    //openArgs[0] = filename and aopenA[1] = mode
                    FileTableEntry entry = fs.open(openArgs[0], openArgs[1]);
                    //if the entry doe snot contain anything then there is a error
                    if(entry != null){
                        return myTcb.getFd(entry);
                    }
                } 
               //if the TCB is null then we return an error
               return ERROR;
                
               case CLOSE:   // to be implemented in project
               //Note to self: boolean close( FileTableEntry ftEnt )
                if((myTcb = scheduler.getMyTcb()) != null){
                    //get the file entry from the param argument
                    FileTableEntry entry = myTcb.getFtEnt(param);
                  
                    boolean close = fs.close(entry);
                    //if we sucessfully close then it should be true
                    if(close == true && entry != null){
               ...
               //Note to self: int fsize( FileTableEntry ftEnt )
                if((myTcb = scheduler.getMyTcb()) != null){
                    //get the table entry from param argument
                    FileTableEntry entry = myTcb.getFtEnt(param);
                    if(entry != null){
               ...
               //Note to self: int seek( FileTableEntry ftEnt, int offset, int whence )
                if((myTcb = scheduler.getMyTcb()) != null){
                    //stores the seek arguments from args
                    int[] seekArgs = (int[]) args;
                    FileTableEntry entry = myTcb.getFtEnt(param);
                    if(entry != null){
                        //seekArgs[0] = offset and seekArgs[1] = whence
                        return(fs.seek(entry, seekArgs[0], seekArgs[1]));
                    }
                }
               //if the TCB is null then we return an error
               return ERROR;
               ...
               //Note to self: boolean delete( String filename )
