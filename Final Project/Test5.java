/*Description: This is a test class given from assignment. This is used to test our file system and its system calls
such as format, open, read, write, seek, closem and delete. All of these system calls will be further described
in the file system class. 
Note: I did not implement this test. 
*/
class Test5 extends Thread {
  ...
  private boolean test5( ) {
    ...
    SysLib.cout("openning..... \n");
    fd = SysLib.open( "css430", "r" );
    SysLib.cout("test5 fd = " + fd + "\n");
  ...
  private boolean test12( ) {
    ...
    for ( int i = 0; i < 6656; i++ )
      if ( tmpBuf[i] != buf6656[i] ) {
        SysLib.cout("Actual buf[" + i + "] = " + buf6656[i] + "\n");
        SysLib.cout( "buf[" + i + "] = " + tmpBuf[i] + " (wrong)\n" );
        SysLib.close( fd );
        return false;
      }
